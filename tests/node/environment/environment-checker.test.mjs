...
test( 'parseSemver and compareSemver parse and compare versions accurately', () => {
	assert.deepEqual( parseSemver( '24.16.1' ), {
		major: 24,
		minor: 16,
		patch: 1,
	} );
	assert.deepEqual( parseSemver( 'v11.0.0' ), {
		major: 11,
		minor: 0,
		patch: 0,
	} );
	assert.deepEqual( parseSemver( '8.3.33-cli' ), {
		major: 8,
		minor: 3,
		patch: 33,
	} );
...
test( 'checkDockerCli returns OS-tailored guidance for WSL, Windows, macOS, Linux', () => {
	...
	const winRes = checkDockerCli( failRunner, {
		id: 'windows',
		isWsl: false,
	} );
	...
	const linuxRes = checkDockerCli( failRunner, {
		id: 'linux',
		isWsl: false,
	} );
...
test( 'checkDockerCompose validates Compose v2 and handles legacy fallback', () => {
	...
	// Modern v2 or newer (e.g. Compose v5)
	...
	const v5Res = checkDockerCompose( v5Runner, plat, true );
	assert.equal( v5Res.status, 'pass' );
...
test( 'checkFilesystemSanity warns on WSL Windows mounts and spaces', () => {
	...
	// WSL mount warning
	const mntRes = checkFilesystemSanity(
		'/mnt/c/Users/black/project',
		wslPlat
	);
	...
	const homeRes = checkFilesystemSanity(
		'/home/black/workspace/project',
		wslPlat
	);
	assert.equal( homeRes.status, 'pass' );

	// Windows path with spaces warning
	const spaceRes = checkFilesystemSanity(
		'C:\\Users\\Black Folder\\project',
		winPlat
	);
...
test( 'checkProjectArtifacts checks node_modules, vendor, and .env', () => {
	const mockExists = ( filePath ) => {
		...
		if ( filePath.endsWith( 'autoload.php' ) ) {
			return false;
		}
		if ( filePath.endsWith( '@wordpress/env' ) ) {
			return true;
		}
		return false;
	};
...
test( 'runEnvironmentCheck integrates checks and computes readiness', async () => {
	const mockRunner = ( cmd, args ) => {
		...
		if ( cmd === 'docker' && args[ 0 ] === '--version' ) {
			return { status: 0, stdout: 'Docker version 27.0.0\n', stderr: '' };
		}
		...
		if ( cmd === 'docker' && args[ 0 ] === 'compose' ) {
			return {
				status: 0,
				stdout: 'Docker Compose version v2.25.0\n',
				stderr: '',
			};
		}
		if ( cmd === 'php' ) {
			return { status: 0, stdout: 'PHP 8.3.0\n', stderr: '' };
		}
		if ( cmd === 'composer' ) {
			return {
				status: 0,
				stdout: 'Composer version 2.7.0\n',
				stderr: '',
			};
		}

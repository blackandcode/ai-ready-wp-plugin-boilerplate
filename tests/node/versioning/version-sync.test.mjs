...
const changelogCliPath = join(projectRoot, 'scripts/record-unreleased-change.mjs');
...
function runCliWithoutEnv(root, args = []) {
  return spawnSync(process.execPath, [cliPath, '--root', root, '--env', 'nonexistent.env', ...args], {
    cwd: projectRoot,
    encoding: 'utf8',
  });
}
...
function runChangelogCli(root, args = []) {
  return spawnSync(process.execPath, [changelogCliPath, '--root', root, ...args], {
    cwd: projectRoot,
    encoding: 'utf8',
  });
}
...
test('updates version via positional CLI argument without .env', async () => {
  const root = await createFixture();
  try {
    const result = runCliWithoutEnv(root, ['1.4.0', '--date', '2026-09-08']);
    assert.equal(result.status, 0, result.stderr);
...
test('calculates patch bump via --bump patch without .env', async () => {
  const root = await createFixture();
  try {
    const result = runCliWithoutEnv(root, ['--bump', 'patch', '--date', '2026-09-08']);
    assert.equal(result.status, 0, result.stderr);
...
test('calculates minor bump via positional argument without .env', async () => {
  const root = await createFixture();
  try {
    const result = runCliWithoutEnv(root, ['minor', '--date', '2026-09-08']);
    assert.equal(result.status, 0, result.stderr);
    assert.match(result.stdout, /1\.2\.3 -> 1\.3\.0/);
...
test('calculates major bump via --bump major without .env', async () => {
  const root = await createFixture();
  try {
    const result = runCliWithoutEnv(root, ['--bump', 'major', '--date', '2026-09-08']);
    assert.equal(result.status, 0, result.stderr);
...
test('recordUnreleasedChange pure function adds entries under correct category', () => {
  ...
  const updated2 = recordUnreleasedChange(updated1, { type: 'Added', message: 'Second new feature' });
  assert.match(updated2, /- First new feature\n- Second new feature/);

  const updated3 = recordUnreleasedChange(updated2, { type: 'Fixed', message: 'Resolved edge case bug' });
  assert.match(updated3, /### Added\n\n- First new feature\n- Second new feature\n\n### Fixed\n\n- Resolved edge case bug/);
});
...
test('record-unreleased-change CLI helper appends bullets and packages cleanly on release', async () => {
  ...
  try {
    ...
    const addResult = runChangelogCli(root, ['-t', 'Fixed', 'Fixed admin sidebar toggle in RTL mode']);
    assert.equal(addResult.status, 0, addResult.stderr);
    assert.match(addResult.stdout, /Successfully recorded unreleased/);

    const changelogAfterAdd = await readFile(join(root, 'CHANGELOG.md'), 'utf8');
    assert.match(changelogAfterAdd, /Fixed admin sidebar toggle in RTL mode/);

    // Now execute a patch release
    const releaseResult = runCliWithoutEnv(root, ['patch', '--date', '2026-09-08']);
    assert.equal(releaseResult.status, 0, releaseResult.stderr);

    const changelogAfterRelease = await readFile(join(root, 'CHANGELOG.md'), 'utf8');
    ...
    assert.match(changelogAfterRelease, /Fixed admin sidebar toggle in RTL mode/);
    // Unreleased section is empty at top
    assert.match(changelogAfterRelease, /## \[Unreleased\]\n\n## \[1\.2\.4\]/);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

...
` );
	...
	assert.equal( isPathIgnored( '.github/workflows/ci.yml', false, rules ), true );
	assert.equal( isPathIgnored( '.cursor/rules/some.mdc', false, rules ), true );
	assert.equal( isPathIgnored( 'docs/devops/releasing-and-distribution.md', false, rules ), true );
	assert.equal( isPathIgnored( 'tests/node/test.mjs', false, rules ), true );
	assert.equal( isPathIgnored( 'tools/release/build.mjs', false, rules ), true );
	assert.equal( isPathIgnored( 'node_modules/react/index.js', false, rules ), true );
	...
	// Must NOT be ignored
	assert.equal( isPathIgnored( 'ai-ready-wp-plugin-boilerplate.php', false, rules ), false );
	assert.equal( isPathIgnored( 'src/framework/Kernel/Plugin.php', false, rules ), false );
	assert.equal( isPathIgnored( 'src/backend/Apps/Settings/Settings.php', false, rules ), false );
	assert.equal( isPathIgnored( 'build/index.js', false, rules ), false );
	assert.equal( isPathIgnored( 'build/index.asset.php', false, rules ), false );
	assert.equal( isPathIgnored( 'vendor/autoload.php', false, rules ), false );
	assert.equal( isPathIgnored( 'vendor/composer/autoload_psr4.php', false, rules ), false );

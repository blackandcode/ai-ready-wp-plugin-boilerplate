...
import { detectCurrentPlugin, scaffoldPlugin } from '../../../scripts/lib/scaffold-engine.mjs';
...
test('scaffoldPlugin performs atomic renaming and token replacement', async () => {
  ...
  try {
    ...
    const mainPhp = await readFile(join(root, 'diagram-pro.php'), 'utf8');
...
test('scaffold CLI supports --dry-run without modifying files', async () => {
  ...
  try {
    ...
    const packageAfter = await readFile(join(root, 'package.json'), 'utf8');
...
test('scaffoldPlugin replaces hyphenated prefix, PascalCase, camelCase tokens, and dynamic REST/block tokens', async () => {
  ...
  try {
    ...
    const controllerContent = await readFile(join(root, 'src/Rest/Controller/HelloWorldController.php'), 'utf8');
    assert.match(controllerContent, /dp\/v1/);

    const blockJsonContent = await readFile(join(root, 'blocks/hello-world/block.json'), 'utf8');

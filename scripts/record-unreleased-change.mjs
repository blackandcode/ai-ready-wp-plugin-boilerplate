...
export function recordUnreleasedChange(source, { type = 'Changed', message }) {
  ...
  if (unreleasedIndex === -1) {
    ...
    const insertAt = firstLineEnd === -1 ? working.length : firstLineEnd + 1;
...
export async function addUnreleasedEntry({
  ...
  return { path: changelogPath, type: normalizeChangeType(type), message: message.trim() };
}
...
if (process.argv[1] && process.argv[1].endsWith('record-unreleased-change.mjs')) {
  ...
  if (!message) {
    console.error('Error: Change message is required. Example: npm run changelog:add -- -t Added "New REST route"');
    process.exit(1);
  }

...
export function updateChangelog(source, { currentVersion, targetVersion, date, message }) {
  ...
  const fallbackBody = [
    '### Changed',
    '',
    `- ${message || `Synchronized plugin version from \`${currentVersion}\` to \`${targetVersion}\` using \`npm run update-version\`.`}`,
  ].join('\n');
...
export async function synchronizeVersion({
  ...
  if (comparison === 0) {
    return {
      ...
      date: effectiveDate,
      scannedFiles: 0,
      phpVersionOrdering: { checked: false, reason: 'Versions are identical.' },
    };
  }

  if (comparison < 0 && !allowDowngrade) {
    throw new Error(`Target version ${targetVersion} is lower than current version ${currentVersion}. Use --allow-downgrade to proceed.`);
  }

  const phpVersionOrdering = verifyPhpVersionOrdering(currentVersion, targetVersion);

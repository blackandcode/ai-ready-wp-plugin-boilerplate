 ...
 * @returns {{ id: 'windows'|'macos'|'wsl'|'linux'|string, name: string, isWsl: boolean, arch: string, platform: string }}
 */
export function detectPlatform(options = {}) {
  ...
  if (plat === 'linux') {
    ...
    if (isWsl) {
      const distro = env.WSL_DISTRO_NAME ? ` (${env.WSL_DISTRO_NAME})` : '';
      return {
        id: 'wsl',
...
export function checkNode(runner, platform, minVersion = '24.16.0') {
  ...
  return {
    ...
    message: ok
      ...
      : `Node.js ${current ? `v${current}` : 'missing'} is below requirement (>= ${minVersion})`,
    remediation,
  };
}
...
export function checkNpm(runner, platform, minVersion = '11.0.0') {
  ...
  const ok = res.status === 0 && parsed && satisfiesMinVersion(output, minVersion);
  ...
  return {
    ...
    message: ok
      ...
      : `npm ${output ? `v${output}` : 'missing'} is below requirement (>= ${minVersion})`,
    remediation,
  };
}
...
export function checkGit(runner, platform) {
  ...
  if (!ok) {
    if (platform.id === 'macos') {
      remediation = 'Install Git via Xcode Command Line Tools:\n  xcode-select --install\nor via Homebrew:\n  brew install git';
    } else if (platform.id === 'windows') {
      remediation = 'Install Git for Windows via winget:\n  winget install Git.Git\nor download from https://git-scm.com/download/win';
    } else {
  ...
  return {
    ...
    message: ok ? output.split('\n')[0] : 'Git command not found in PATH',
    remediation,
  };
}
...
export function checkDockerCli(runner, platform) {
  ...
  return {
    ...
    found: displayFound,
    required: 'Installed and available in PATH',
    message: ok ? output.split('\n')[0] : 'Docker command not found in PATH',
    remediation,
  };
}
...
export function checkDockerDaemon(runner, platform, dockerCliPassed) {
  if (!dockerCliPassed) {
    return {
      ...
      message: 'Docker daemon could not be verified because Docker CLI is not found',
      remediation: 'Resolve Docker CLI installation first.',
    };
  }

  const res = runner('docker', ['info', '--format', '{{.ServerVersion}}']);
  const output = (res.stdout || '').trim();
  const ok = res.status === 0 && output.length > 0 && !res.stderr?.includes('permission denied');

  let remediation = '';
  if (!ok) {
    const errText = (res.stderr || res.stdout || '').toLowerCase();
    if (errText.includes('permission denied') && (platform.id === 'linux' || platform.id === 'wsl')) {
      remediation =
  ...
  return {
    ...
    message: ok ? `Docker daemon is active (Server v${output})` : 'Docker daemon is not running or socket is inaccessible',
    remediation,
  };
}
...
export function checkDockerCompose(runner, platform, dockerCliPassed) {
  if (!dockerCliPassed) {
    return {
      ...
      message: 'Docker Compose check skipped because Docker CLI is missing',
      remediation: 'Install Docker Desktop or Docker Compose plugin.',
    };
  }

  // First try modern "docker compose version"
  let res = runner('docker', ['compose', 'version']);
  let output = (res.stdout || '').trim();
  let isV2 = res.status === 0 && /version\s+v?2\./i.test(output);
  ...
  if (!isV2) {
    ...
    if (fallbackRes.status === 0 && /version\s+v?2\./i.test(fallbackOutput)) {
      isV2 = true;
...
export function checkPhp(runner, platform, minVersion = '8.3.0') {
  ...
  const ok = res.status === 0 && parsed && satisfiesMinVersion(firstLine, minVersion);
  ...
  return {
    ...
    message: ok
      ...
      : `Host PHP ${firstLine ? `(${firstLine})` : 'missing'}: WP runs in Docker (PHP 8.3), but host PHP 8.3 is needed for 'composer lint' and 'composer test'`,
    remediation,
  };
}
...
export function checkComposer(runner, platform, minVersion = '2.7.0') {
  ...
  if (!ok) {
    if (platform.id === 'macos') {
      remediation = 'Install Composer via Homebrew:\n  brew install composer';
    } else if (platform.id === 'windows') {
      remediation = 'Install Composer via Scoop:\n  scoop install composer\nor download Windows installer from https://getcomposer.org/download/';
    } else {
      remediation =
...
export function checkFilesystemSanity(projectRoot, platform) {
  ...
  if (platform.id === 'wsl') {
    if (normalized.startsWith('/mnt/')) {
      return {
        ...
        message: 'Project is located inside Windows 9P mount (/mnt/c/...). This causes severe I/O latency and breaks Webpack/Playwright file watchers.',
        remediation:
    ...
    return {
      ...
      found: projectRoot,
      required: 'Linux root filesystem (/home/<user>/...)',
      message: 'Project resides in native Linux filesystem; optimal I/O and inotify watcher performance.',
      remediation: '',
    };
  }
  ...
  if (/\s/.test(projectRoot)) {
    return {
      ...
      message: 'Project path contains whitespace. Some Docker volume mounts and shell tools can misbehave with unquoted paths.',
      remediation: 'Consider cloning the repository into a path without spaces if you encounter volume mount errors.',
    };
  }
 ...
 * @param {number} port
 * @param {string} [host]
 * @returns {Promise<boolean>} True if port is free (listening succeeded), false if occupied
 */
...
export async function checkPorts(portTester = testPortAvailable) {
  ...
  if (!wpFree || !pmaFree) {
    const occupied = [];
    if (!wpFree) occupied.push(`port ${wpPort} (WordPress)`);
    if (!pmaFree) occupied.push(`port ${pmaPort} (phpMyAdmin)`);
    ...
    return {
      ...
      message: `${occupied.join(' and ')} currently occupied by another process or existing wp-env container`,
      remediation:
  ...
  return {
    ...
    message: 'Ports 8888 (WordPress) and 8890 (phpMyAdmin) are available for wp-env.',
    remediation: '',
  };
}
...
export function checkProjectArtifacts(projectRoot, fileExists = existsSync) {
  ...
  // 1. node_modules
  const hasWpEnv = fileExists(resolve(projectRoot, 'node_modules/@wordpress/env'));
  ...
  if (hasWpEnv) {
    ...
    checks.push({
      ...
      category: 'project',
      status: 'warn',
      found: hasNodeModules ? 'Incomplete node_modules' : 'Missing node_modules',
      required: 'npm dependencies installed',
      message: 'npm dependencies are missing or incomplete. Run npm install before starting development.',
      remediation: 'Run:\n  npm install',
    });
  }

  // 2. vendor (Composer)
  const hasVendorAutoload = fileExists(resolve(projectRoot, 'vendor/autoload.php'));
  if (hasVendorAutoload) {
    ...
    checks.push({
      ...
      required: 'Composer vendor directory installed',
      message: 'Composer dependencies not installed on host. Run composer install for local PHP tooling.',
      remediation: 'Run:\n  composer install',
    });
  }
  ...
  if (hasEnv) {
    ...
    checks.push({
      ...
      message: 'Local .env file not found. Copy .env.example to .env to enable API and Playwright credentials.',
      remediation: hasEnvExample ? 'Run:\n  cp .env.example .env' : 'Create a .env file with WP_BASE_URL=http://localhost:8888',
    });
  }
...
export async function runEnvironmentCheck(options = {}) {
  ...
  const dockerDaemonCheck = checkDockerDaemon(runner, platform, dockerCliPassed);
  const dockerComposeCheck = checkDockerCompose(runner, platform, dockerCliPassed);

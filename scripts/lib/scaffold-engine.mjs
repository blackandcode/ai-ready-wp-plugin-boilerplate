...
export async function detectCurrentPlugin(root) {
  ...
  if (await fileExists(packagePath)) {
    ...
    if (pkg.name) currentSlug = pkg.name;
    if (pkg.description) currentDescription = pkg.description;
  }
  ...
  for await (const entry of glob(['*.php'], { cwd: root, followSymlinks: false })) {
    ...
    if (content.includes('Plugin Name:')) {
      ...
      if (authorMatch) currentAuthor = authorMatch[1].trim();
      break;
    }
  }
  ...
  if (await fileExists(join(root, mainPhpFile))) {
    ...
    const prefixMatch = content.match(/define\(\s*['"]([A-Z0-9_]+?)(?:PLUGIN_FILE|PLUGIN_DIR|PLUGIN_URL|VERSION)['"]/);
    if (prefixMatch && prefixMatch[1]) {
      currentPrefix = prefixMatch[1].endsWith('_') ? prefixMatch[1] : `${prefixMatch[1]}_`;
    }
  }
  ...
  for (const candidate of restCandidates) {
    if (await fileExists(candidate)) {
      ...
      const restMatch = content.match(/protected\s+\$namespace\s*=\s*['"]([^'"]+)['"]/);
  ...
  // Detect current block name from block.json
  let currentBlockName = 'ai-ready-wp/hello-world';
  const blockJsonPath = join(root, 'blocks/hello-world/block.json');
  ...
  const currentLowerHyphenPrefix = currentPrefix.toLowerCase().replace(/_+$/, '').replace(/_/g, '-') + '-';
...
export async function scaffoldPlugin({
  ...
  const targetNamespace = namespace || `${toPascalCase(targetAuthor)}\\${toPascalCase(targetSlug)}`;
  let targetPrefix = (prefix || `${targetSlug.replace(/[^a-zA-Z0-9]/g, '_').toUpperCase()}_`).trim();
  if (!targetPrefix.endsWith('_')) {
    targetPrefix += '_';
  }
  const targetTextDomain = textDomain || targetSlug;
  const targetDescription = description || `Production-ready WordPress plugin built with the Agentic Boilerplate.`;
  ...
  const targetLowerHyphenPrefix = targetPrefix.toLowerCase().replace(/_+$/, '').replace(/_/g, '-') + '-';
  ...
  const replacements = [
    ...
    ...(currentComposerName && targetComposerName && currentComposerName !== targetComposerName
    ...
    ...(current.lowerHyphenPrefix !== targetLowerHyphenPrefix
      ? [{ from: current.lowerHyphenPrefix, to: targetLowerHyphenPrefix }]
      : []),
    ...
    ...(current.camelPrefix !== targetCamelPrefix
      ? [{ from: `${current.camelPrefix}AdminBootstrap`, to: `${targetCamelPrefix}AdminBootstrap` }]
      : []),
    // 13. Slug / Textdomain
    {
      from: current.slug,
  ...
  if (oldMainPhpPath !== newMainPhpPath && (await fileExists(oldMainPhpPath))) {
    fileRenames.push({
      from: oldMainPhpPath,
      to: newMainPhpPath,
  ...
  for await (const entry of glob(['**/*', '.*', '**/.*'], {
    ...
    if (projectPath.startsWith('.git/') || projectPath.startsWith('node_modules/') || projectPath.startsWith('vendor/')) {
      continue;
    }
    ...
    for (const rule of replacements) {
      if (rule.from && rule.from !== rule.to && content.includes(rule.from)) {
  ...
  if (!dryRun) {
    ...
    try {
      ...
      for (const [absPath, change] of changes) {
        ...
        await writeFile(tempPath, change.after, { encoding: 'utf8', mode: fileState.mode });
        await chmod(tempPath, fileState.mode);
        await rename(tempPath, absPath);
        committedFiles.push({ path: absPath, before: change.before, mode: fileState.mode });
      }
      ...
      for (const file of committedFiles.reverse()) {
        await writeFile(file.path, file.before, { encoding: 'utf8', mode: file.mode });
      }
      throw new Error(`Scaffolding failed and changes were rolled back: ${err.message}`);
    }
  }
  ...
  return {
    ...
    renamedFiles: fileRenames.map((r) => ({ from: r.relativeFrom, to: r.relativeTo })),
  };
}

...
function testSkillSpecification() {
	...
	assert(
		lines.length < 500,
		`SKILL.md is under 500 lines (actual: ${ lines.length } lines)`
	);

	const fm = parseFrontmatter( content );
	assert( fm !== null, 'Valid YAML frontmatter present' );
	assert(
		fm.name === 'wp-architecture-decision-records',
		`Skill name matches directory ('wp-architecture-decision-records')`
	);
	assert(
		fm.description && fm.description.length <= 1024,
		`Description present and <= 1024 chars (${ fm.description?.length } chars)`
	);
	assert(
		fm.compatibility && fm.compatibility.includes( 'WordPress' ),
		'Compatibility indicates WordPress target'
	);
}
...
function testScenariosSuite() {
	...
	assert(
		scenarioFiles.length === 12,
		`Exactly 12 evaluation scenarios found (actual: ${ scenarioFiles.length })`
	);
	...
	for ( const file of scenarioFiles ) {
		...
		assert(
			Boolean( parsed.classification ),
			`${ file }: contains 'classification'`
		);
		assert( Boolean( parsed.query ), `${ file }: contains 'query'` );
		assert(
			Array.isArray( parsed.expected_behavior ) &&
				parsed.expected_behavior.length > 0,
			`${ file }: contains non-empty expected_behavior[]`
		);
		assert(
			Array.isArray( parsed.success_criteria ) &&
				parsed.success_criteria.length > 0,
			`${ file }: contains non-empty success_criteria[]`
		);
	}
}
...
function testDeterministicScriptsE2E() {
	...
	assert(
		typeof triage === 'object' && triage.rootPath,
		'detectProject returns structured analysis'
	);
	...
	try {
		...
		assert(
			conv0.hasAdrDirectory === false,
			'Initially detects no ADR directory'
		);
		assert(
			conv0.nextFormattedNumber === '0001',
			'Recommends 0001 as first number'
		);
		...
		assert(
			fs.existsSync( path.join( tempDir, 'docs/adr/README.md' ) ),
			'README.md created'
		);
		assert(
			fs.existsSync(
		...
		assert(
			adr2.formattedNumber === '0002',
			'ADR-0002 formatted number matches'
		);
		assert(
			adr2.indexUpdated === true,
			'ADR-0002 appended to index table'
		);
		...
		assert(
			val1.valid === true,
			'validateAdrs passes on bootstrapped + new ADR'
		);
		...
		assert(
			mut.success === true,
			'setAdrStatus successfully supersedes ADR-0002'
		);
		assert(
			mut.indexUpdated === true,
			'Index updated with superseded status'
		);
		...
		assert(
			val2.valid === true,
			'validateAdrs passes after bidirectional supersession'
		);
		assert( val2.totalChecked === 3, 'validateAdrs checked 3 records' );

		// 8. Negative test: Introduce broken link in ADR-0003
		const adr3Path = path.join(
			tempDir,
			'docs/adr/0003-migrate-webhooks-to-custom-db-queue.md'
		);
		let adr3Content = fs.readFileSync( adr3Path, 'utf8' );
		adr3Content = adr3Content.replace(
			/Supersedes:\s*.*$/m,
			'Supersedes: [ADR-9999](9999-ghost.md)'
		);
		...
		assert(
			valFail.valid === false,
			'validateAdrs correctly catches broken supersedes link'
		);
		assert(
			valFail.files.some( ( f ) =>
				f.errors.some( ( e ) => e.includes( '9999-ghost.md' ) )
			),
			'Identifies specific broken link error'
		);
	} finally {
		// Cleanup temp dir
		try {
...
function main() {
	process.stdout.write(
		`Starting Evaluation Suite for 'wp-architecture-decision-records'...\n`
	);
	...
	try {
		...
		process.stdout.write(
			`🎉 ALL EVALUATION SUITES PASSED! (${ passedTests }/${ totalTests } assertions, ${ elapsed }ms)\n`
		);
		process.stdout.write(
			`======================================================\n`
		);

...
function printHelp() {
  process.stdout.write(`Usage: node detect-project.mjs [options] [target-directory]
...
function findFiles(dir, filterFn, maxDepth = 4, currentDepth = 0) {
  ...
  try {
    ...
    for (const entry of entries) {
      if (['node_modules', 'vendor', '.git', 'build', 'dist', 'coverage'].includes(entry.name)) {
        continue;
      }
      const fullPath = path.join(dir, entry.name);
      if (entry.isDirectory()) {
        results.push(...findFiles(fullPath, filterFn, maxDepth, currentDepth + 1));
...
function detectPluginHeader(content) {
  ...
  const match = content.match(/\/\*[\s\S]*?Plugin Name:\s*([^\r\n]+)[\s\S]*?\*\//i);
...
function detectWordPressPlugin(targetDir) {
  ...
  const phpFiles = findFiles(targetDir, (name) => name.endsWith('.php'), 1);
  let mainPlugin = null;
...
function analyzeComposer(targetDir) {
  ...
  try {
    ...
    return {
      ...
      hasPhpstan: Boolean(json['require-dev']?.['phpstan/phpstan'] || json['require-dev']?.['szepeviktor/phpstan-wordpress']),
      hasPhpcs: Boolean(json['require-dev']?.['squizlabs/php_codesniffer'] || json['require-dev']?.['wp-coding-standards/wpcs']),
      hasPhpunit: Boolean(json['require-dev']?.['phpunit/phpunit']),
      dependencies: Object.keys(json.require || {}).filter((k) => k !== 'php'),
...
function analyzePackageJson(targetDir) {
  ...
  try {
    ...
    return {
      ...
      hasWpScripts: Boolean(json.dependencies?.['@wordpress/scripts'] || json.devDependencies?.['@wordpress/scripts']),
      hasPlaywright: Boolean(json.dependencies?.['@playwright/test'] || json.devDependencies?.['@playwright/test']),
...
function detectArchitecturalCharacteristics(targetDir) {
  ...
  const phpFiles = findFiles(targetDir, (name) => name.endsWith('.php'), 4);
  for (const file of phpFiles) {
    const content = readFileSafe(file, 64 * 1024);
    if (!content) continue;

    if (!characteristics.hasCustomTables && (content.includes('dbDelta(') || content.includes('CREATE TABLE'))) {
      characteristics.hasCustomTables = true;
    }
    if (!characteristics.hasActionScheduler && (content.includes('as_schedule_') || content.includes('as_enqueue_') || content.includes('ActionScheduler'))) {
      characteristics.hasActionScheduler = true;
    }
    if (!characteristics.hasWpCron && (content.includes('wp_schedule_event') || content.includes('wp_next_scheduled'))) {
      characteristics.hasWpCron = true;
    }
    if (!characteristics.hasRestApi && (content.includes('register_rest_route') || content.includes('WP_REST_Controller'))) {
      characteristics.hasRestApi = true;
    }
    if (!characteristics.hasWooCommerce && (content.includes('woocommerce_') || content.includes('WC_') || content.includes('Automattic\\WooCommerce'))) {
      characteristics.hasWooCommerce = true;
    }
    if (!characteristics.hasCustomPostTypes && (content.includes('register_post_type') || content.includes('register_taxonomy'))) {
      characteristics.hasCustomPostTypes = true;
    }
    if (!characteristics.hasDependencyInjection && (content.includes('Container') || content.includes('ServiceProvider') || content.includes('DependencyInjection'))) {
      characteristics.hasDependencyInjection = true;
    }
  }

  // Check for blocks
  const blockJsonFiles = findFiles(targetDir, (name) => name === 'block.json', 3);
...
function detectExistingAdrs(targetDir) {
  const candidateDirs = ['docs/adr', 'docs/decisions', 'adr', 'decisions', '.adr'];
  let foundDir = null;
  let adrFiles = [];
  ...
  for (const cand of candidateDirs) {
    ...
    if (fs.existsSync(full) && fs.statSync(full).isDirectory()) {
      foundDir = cand;
      adrFiles = fs.readdirSync(full).filter((f) => f.endsWith('.md') && !f.toLowerCase().includes('readme'));
      break;
    }
  }
...
function main() {
  ...
  if (args.json) {
    ...
    process.stdout.write(`WordPress Plugin: ${result.isWordPressPlugin ? 'YES' : 'NO'}\n`);
    if (result.pluginHeader) {
      process.stdout.write(`  Name: ${result.pluginHeader.name}\n`);
      process.stdout.write(`  Entry File: ${result.pluginHeader.filePath}\n`);
      process.stdout.write(`  Version: ${result.pluginHeader.version || 'unknown'}\n`);
      process.stdout.write(`  Requires PHP: ${result.pluginHeader.requiresPhp || 'unknown'}\n`);
      process.stdout.write(`  Requires WP: ${result.pluginHeader.requiresAtLeast || 'unknown'}\n`);
    }
    if (result.composer) {
      process.stdout.write(`Composer: YES (PHP ${result.composer.phpConstraint || 'unspecified'})\n`);
      process.stdout.write(`  Dependencies: ${result.composer.dependencies.join(', ') || 'none'}\n`);
    }
    process.stdout.write(`Architecture Signals:\n`);
    for (const [key, val] of Object.entries(result.architecture)) {
      if (val) process.stdout.write(`  - ${key}: detected\n`);
    }
    process.stdout.write(`Existing ADRs:\n`);
    process.stdout.write(`  Has ADRs: ${result.adrs.hasAdrs ? 'YES' : 'NO'}\n`);
    if (result.adrs.adrDirectory) {
      process.stdout.write(`  Directory: ${result.adrs.adrDirectory} (${result.adrs.adrCount} records)\n`);
    }
  }
}

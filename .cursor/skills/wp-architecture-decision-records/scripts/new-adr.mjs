...
function printHelp() {
	process.stdout
		.write( `Usage: node new-adr.mjs --title "<Decision Title>" [options]
...
export function createNewAdr( targetDir = '.', options = {} ) {
	...
	if ( ! title ) {
		return {
			success: false,
			error: 'A decision title is required (e.g. --title "Adopt Action Scheduler")',
		};
	}

	const conventions = detectAdrConventions( absRoot, options.dir );
	const adrRelDir = conventions.hasAdrDirectory
		? conventions.adrDirectory
		: options.dir || 'docs/adr';
	...
	if ( fs.existsSync( absFilePath ) ) {
		return {
			success: false,
			error: `File collision: '${ relFilePath }' already exists.`,
		};
	}
	...
	if ( ! dryRun ) {
		...
		if ( options.updateIndex && conventions.indexFile ) {
			...
			const row = formatIndexRow(
				...
				status,
				today,
				supersedes !== 'None' ? supersedes : null
			);
...
function main() {
	...
	if ( ! args.title ) {
		process.stderr.write(
			'ERROR: Missing required option --title (e.g. --title "Adopt Action Scheduler")\n'
		);
		process.exit( 1 );
	}

...
function printHelp() {
  process.stdout.write(`Usage: node detect-adr-conventions.mjs [options] [target-directory]
...
function parseAdrMetadata(content) {
  ...
  // Template heuristic
  const hasOptions = content.includes('## Considered Options') || content.includes('## Decision Drivers');
...
export function detectAdrConventions(targetDir = '.', customDir = null) {
  ...
  for (const entry of rawEntries) {
    ...
    if (indexFile && entry.name.toLowerCase() === indexFile.toLowerCase()) continue;
    ...
    if (numMatch) {
      ...
      if (numStr.length === 3) detectedPadLength = 3;
      else if (numStr.length === 4) detectedPadLength = 4;
    }

    const content = fs.readFileSync(path.join(absAdrDir, entry.name), 'utf8');
  ...
  const nextFormattedNumber = String(nextNumber).padStart(detectedPadLength, '0');
  ...
  const stats = {
    ...
    deprecated: adrFiles.filter((f) => f.status === 'deprecated').length,
    superseded: adrFiles.filter((f) => f.status === 'superseded').length,
    unknown: adrFiles.filter((f) => !['proposed', 'accepted', 'rejected', 'deprecated', 'superseded'].includes(f.status)).length,
  };

  return {
    hasAdrDirectory: true,
    adrDirectory: relAdrDir,
    indexFile: indexFile ? toPosix(path.join(relAdrDir, indexFile)) : null,
    numberingStyle,
    detectedPadLength,
    highestNumber,
    nextNumber,
    nextFormattedNumber,
    stats,
    files: adrFiles.sort((a, b) => (a.number || 0) - (b.number || 0)),
  };
}
...
function main() {
  ...
  if (args.json) {
    ...
    process.stdout.write(`ADR Directory Exists: ${result.hasAdrDirectory ? 'YES' : 'NO'}\n`);
    if (result.hasAdrDirectory) {
      process.stdout.write(`Directory: ${result.adrDirectory}\n`);
      process.stdout.write(`Index File: ${result.indexFile || 'none'}\n`);
      process.stdout.write(`Numbering Style: ${result.numberingStyle} (padding: ${result.detectedPadLength})\n`);
      process.stdout.write(`Highest Sequence: ${result.highestNumber}\n`);
      process.stdout.write(`Next Sequence: ${result.nextFormattedNumber} (${result.nextNumber})\n`);
      ...
      process.stdout.write(`  Superseded: ${result.stats.superseded}\n`);
      process.stdout.write(`  Deprecated: ${result.stats.deprecated}\n`);
      process.stdout.write(`  Rejected: ${result.stats.rejected}\n`);
    } else {
      process.stdout.write(`Recommended Directory: ${result.recommendedDirectory}\n`);
      process.stdout.write(`Recommended Next Sequence: ${result.nextFormattedNumber}\n`);
    }
  }
}

...
function printHelp() {
	process.stdout
		.write( `Usage: node set-adr-status.mjs --adr <id|path> --status <status> [options]
...
function resolveAdrFile( absRoot, adrInput, adrDir ) {
	...
	const searchDir = conventions.hasAdrDirectory
		? path.join( absRoot, conventions.adrDirectory )
		: path.join( absRoot, 'docs/adr' );
	...
	const files = fs
		...
		.filter(
			( f ) =>
				f.endsWith( '.md' ) && ! f.toLowerCase().includes( 'readme' )
		);
...
export function setAdrStatus( targetDir = '.', options = {} ) {
	...
	if ( ! adrInput ) {
		return {
			success: false,
			error: 'Target ADR is required (--adr <number|filename>).',
		};
	}
	...
	if ( ! targetFile ) {
		return {
			success: false,
			error: `Could not find ADR matching '${ adrInput }'.`,
		};
	}
	...
	if ( newStatus === 'superseded' && options.supersededBy ) {
		const successorFile = resolveAdrFile(
			absRoot,
			options.supersededBy,
			options.dir
		);
		if ( successorFile ) {
			linkedSuccessorFile = toPosix(
				path.relative( absRoot, successorFile )
			);
	...
	if ( options.supersedes ) {
		const priorFile = resolveAdrFile(
			absRoot,
			options.supersedes,
			options.dir
		);
	...
	if ( ! dryRun ) {
		...
		if ( linkedSuccessorFile && options.supersededBy ) {
			...
			if ( fs.existsSync( absSuccessor ) ) {
				...
				const updatedSucc = updateStatusInContent(
					succContent,
					'accepted',
					null,
					backLink,
		...
		if ( options.updateIndex ) {
			...
			if ( conventions.indexFile ) {
				...
				indexUpdated = updateIndexStatus(
					absIndex,
					targetFile,
					newStatus
				);
...
function main() {
	...
	if ( ! args.adr || ! args.status ) {
		process.stderr.write(
			'ERROR: Both --adr and --status are required.\n'
		);
		process.exit( 1 );
	}
	...
	if ( args.json ) {
		...
		if ( result.supersededBy ) {
			process.stdout.write(
				`  Superseded by: ${ result.supersededBy }\n`
			);
		}

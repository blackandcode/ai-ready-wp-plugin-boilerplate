...
const REQUIRED_SECTIONS = [
  ...
  { name: 'Architectural Constraints', regex: /##\s+Architectural Constraints/i },
...
function printHelp() {
  process.stdout.write(`Usage: node validate-adr.mjs [options] [target-directory]
...
function validateSingleAdr(filePath, absAdrDir, options = {}) {
  ...
  if (!titleMatch) {
    errors.push('Missing top-level level 1 heading (e.g. # ADR-0001: Decision Title).');
  } else {
    const headerNumStr = titleMatch[1];
    const fileNumMatch = fileName.match(/^(\d+)-/);

    if (fileNumMatch && headerNumStr) {
      if (parseInt(fileNumMatch[1], 10) !== parseInt(headerNumStr, 10)) {
        errors.push(`Header sequence number (ADR-${headerNumStr}) does not match filename prefix (${fileNumMatch[1]}).`);
      }
    }
  }

  // 2. Status check
  const statusMatch = content.match(/(?:-\s+\*\*Status:\*\*|\bStatus:)\s*([a-zA-Z]+)/i);
  let status = null;
  if (!statusMatch) {
  ...
  // 3. Date check
  const dateMatch = content.match(/(?:-\s+\*\*Date:\*\*|\bDate:)\s*(\d{4}-\d{2}-\d{2})/i);
  if (!dateMatch) {
    errors.push('Missing or invalid Date metadata (expected YYYY-MM-DD).');
  }

  // 4. Required sections
  for (const sec of REQUIRED_SECTIONS) {
    if (!sec.regex.test(content)) {
      errors.push(`Missing required architectural section: '${sec.name}'.`);
    }
  }
  ...
  if (content.includes('<!-- Describe the technical') || content.includes('<!-- What is the problem')) {
    const msg = 'Template instructional comments have not been replaced with actual rationale.';
    if (options.strict) errors.push(msg);
    else warnings.push(msg);
  }
  ...
  if (status === 'superseded') {
    const supByMatch = content.match(/(?:-\s+\*\*Superseded by:\*\*|\bSuperseded by:\s*)([^\r\n]+)/i);
    const rawSupBy = supByMatch ? supByMatch[1] : '';
    const cleanSupBy = rawSupBy.replace(/<!--[\s\S]*?-->/g, '').trim();

    if (!cleanSupBy || cleanSupBy.toLowerCase() === 'none' || cleanSupBy === '—') {
      errors.push("ADR has status 'superseded' but 'Superseded by:' link is missing or set to None.");
    } else {
      // Check link target
      const fileLinkMatch = cleanSupBy.match(/\]\(([^)]+\.md)\)/);
      if (fileLinkMatch && absAdrDir) {
        const targetPath = path.resolve(absAdrDir, fileLinkMatch[1]);
        if (!fs.existsSync(targetPath)) {
          errors.push(`Broken supersession link: target file '${fileLinkMatch[1]}' does not exist.`);
        }
      }
    }
  }
  ...
  const supMatch = content.match(/(?:-\s+\*\*Supersedes:\*\*|\bSupersedes:\s*)([^\r\n]+)/i);
  if (supMatch) {
    const rawSup = supMatch[1];
    const cleanSup = rawSup.replace(/<!--[\s\S]*?-->/g, '').trim();

    if (cleanSup && cleanSup.toLowerCase() !== 'none' && cleanSup !== '—') {
      const fileLinkMatch = cleanSup.match(/\]\(([^)]+\.md)\)/);
      if (fileLinkMatch && absAdrDir) {
        const targetPath = path.resolve(absAdrDir, fileLinkMatch[1]);
        if (!fs.existsSync(targetPath)) {
          errors.push(`Broken supersedes link: prior file '${fileLinkMatch[1]}' does not exist.`);
        }
      }
    }
  }
...
function validateIndexConsistency(absIndexFile, adrResults) {
  ...
  for (const adr of adrResults) {
    ...
    if (!content.includes(baseName)) {
      indexIssues.push(`ADR file '${baseName}' is missing from the index table in ${path.basename(absIndexFile)}.`);
    } else if (adr.status) {
      // Check if status matches in row
      const capStatus = adr.status.charAt(0).toUpperCase() + adr.status.slice(1);
      const rowPattern = new RegExp(`\\[ADR-[^\\]]+\\]\\(${baseName}\\)[^\\n]*\\|\\s*([a-zA-Z]+)\\s*\\|`, 'i');
      const rowMatch = content.match(rowPattern);
      if (rowMatch) {
        const indexStatus = rowMatch[1].toLowerCase().trim();
        if (indexStatus !== adr.status) {
          indexIssues.push(`Status mismatch for '${baseName}': file status is '${adr.status}', but index table records '${indexStatus}'.`);
        }
      }
    }
  }
...
export function validateAdrs(targetDir = '.', options = {}) {
  ...
  for (const entry of rawEntries) {
    ...
    if (conventions.indexFile && entry.name.toLowerCase() === path.basename(conventions.indexFile).toLowerCase()) {
      continue;
    }
  ...
  const allPassed = fileResults.every((r) => r.valid) && indexIssues.length === 0;
...
function main() {
  ...
  if (args.json) {
    ...
    process.stdout.write(`Total records checked: ${result.totalChecked} (Passed: ${result.passed}, Failed: ${result.failed})\n\n`);

    for (const f of result.files) {
      const icon = f.valid ? '✅' : '❌';
      process.stdout.write(`${icon} ${f.fileName} (${f.status || 'unknown'})\n`);

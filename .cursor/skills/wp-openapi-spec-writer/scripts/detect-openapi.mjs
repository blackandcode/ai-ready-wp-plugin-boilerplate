...
const root = candidates.find( ( dir ) => existsSync( join( dir, 'composer.json' ) ) && existsSync( join( dir, 'src' ) ) ) || cwd;
...
function walk( dir ) {
    ...
    for ( const entry of readdirSync( dir, { withFileTypes: true } ) ) {
        ...
        if ( /Controller\.php$/.test( entry.name ) ) {
            ...
            if ( text.includes( 'WP_REST_Controller' ) || text.includes( 'register_rest_route' ) ) {
                controllers.push( relative( root, path ).replaceAll( '\\', '/' ) );
            }
        }
    }
}
...
const result = {
    ...
    openapiSpec: existsSync( specPath ) ? relative( root, specPath ).replaceAll( '\\', '/' ) : null,

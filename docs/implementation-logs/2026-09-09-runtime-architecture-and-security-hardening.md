...
- **Status:** Completed
- **Version Status:** Staged under `## [Unreleased]` in `CHANGELOG.md` (Version `1.4.0`; manual bump via `npm run update-version:minor` or `npm run update-version:patch` when ready)
...
2. **Security Boundary Hardening & Global I/O Policy:** Hardened all REST controllers. Declared explicit schema-level `validate_callback` and `sanitize_callback` rules for all fields on `POST /wpaibp/v1/settings`. Enforced strict `manage_options` capability checks across all privileged endpoints, removed loose cookie authentication fallback bypasses from `DevOpenApiController`, and hardened `uninstall.php` with `$wpdb->prepare()`.

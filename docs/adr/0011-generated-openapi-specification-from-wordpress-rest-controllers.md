...
2. **Deterministic OpenAPI 3.1 Generation:**
   A dedicated development pipeline (`src/development/OpenApi/`, amended from framework per ADR-0013) introspects registered routes for namespace `wpaibp/v1`, converts JSON schemas to OpenAPI 3.1 components, normalizes route regexes to OpenAPI path templates, and outputs deterministic, byte-identical YAML to `docs/api/openapi.yaml`.

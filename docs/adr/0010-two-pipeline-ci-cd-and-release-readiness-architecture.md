...
6. **Supply Chain Hardening:** Immutable GitHub Releases, Sigstore build provenance attestations (`actions/attest-build-provenance`), commit-SHA action pinning, Dependabot automation, and least-privilege token permissions.
...
- Pipeline 1 (`ci.yml`): Runs on every pull request to `main` and push to `main`, invoking a reusable workflow (`_release-readiness.yml`). It lints, runs tests across PHP 8.3 and 8.5, compiles assets, builds the production ZIP via `.distignore`, validates the ZIP against a strict package contract, runs WordPress Plugin Check on the extracted ZIP, and generates a rich `$GITHUB_STEP_SUMMARY`.
- Pipeline 2 (`release.yml`): Manually triggered via `workflow_dispatch` on `main`. It validates version consistency across `package.json`, main plugin file, `AIRWP_VERSION`, `readme.txt`, and `CHANGELOG.md`, runs the exact same `_release-readiness.yml` gate, signs the artifact with Sigstore provenance attestation, drafts the release, uploads the ZIP + SHA256 + changelog notes, and publishes an immutable GitHub Release.
...
Developers push git tags (`git push origin v1.2.0`), which triggers a standalone release workflow that builds the ZIP and publishes the release.
...
- **Risk:** Vendor directory containing dev dependencies or missing production autoloader.
  **Mitigation:** `build-package.mjs` executes `composer install --no-dev --prefer-dist --optimize-autoloader` before packaging, and `validate-package.mjs` verifies `vendor/autoload.php` is present while dev tools are absent.
- **Risk:** Version mismatch across plugin files during release dispatch.
  **Mitigation:** `validate-release.mjs` strictly verifies that the requested release version matches `package.json`, plugin header `Version:`, `AIRWP_VERSION` constant, `readme.txt` `Stable tag:`, and `CHANGELOG.md` entry.
...
1. **Reusable Release-Readiness Gate:** All quality, linting, test, packaging, and Plugin Check steps must reside in `.github/workflows/_release-readiness.yml` (`workflow_call`). Workflows must not define separate release criteria.
2. **Read-Only Release Workflow on `main`:** The release workflow (`.github/workflows/release.yml`) must only execute against `refs/heads/main` and must never commit, push, or modify files in the repository.
...
4. **Package Content Contract:** Every distributable archive must be validated by `tools/release/validate-package.mjs`. It must contain the root plugin directory `{slug}/`, main PHP file, `src/`, `build/`, `vendor/autoload.php`, `readme.txt`, and `uninstall.php`, and must not contain tests, tools, dotfiles, node_modules, or development configurations.
5. **Pinned Action Dependencies:** All external GitHub Actions in `.github/workflows/` must be pinned to full 40-character commit SHAs with version comments (e.g. `actions/checkout@<SHA> # v4.2.2`).
6. **Least-Privilege Security:** Default workflow permissions must be `contents: read`. Elevated scopes (`contents: write`, `attestations: write`, `id-token: write`) are restricted exclusively to the publish job in `release.yml`.

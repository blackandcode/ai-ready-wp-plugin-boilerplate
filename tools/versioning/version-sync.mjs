...
function verifyPhpVersionOrdering( currentVersion, targetVersion ) {
	...
	if ( php.error ) {
		throw new Error(
			`Unable to run PHP version_compare(): ${ php.error.message }`
		);
	}
	if ( php.status !== 0 ) {
		throw new Error(
...
async function readJson( path ) {
	...
	try {
		...
		throw new Error(
			`Cannot parse JSON file ${ path }: ${ error.message }`
		);
	}
}
...
export function updateChangelog(
	source,
	{ currentVersion, targetVersion, date, message }
) {
	if ( source.includes( `## [${ targetVersion }]` ) ) {
		throw new Error(
			`CHANGELOG.md already contains version ${ targetVersion }.`
		);
	}
	...
	const fallbackBody = [
		...
		`- ${
			message ||
			`Synchronized plugin version from \`${ currentVersion }\` to \`${ targetVersion }\` using \`npm run update-version\`.`
		}`,
	].join( '\n' );
	...
	if ( unreleasedIndex === -1 ) {
		...
		const insertionIndex =
			firstHeadingEnd === -1 ? source.length : firstHeadingEnd + 1;
		return `${ source.slice(
			0,
			insertionIndex
		) }\n${ unreleasedHeading }\n\n${ releaseHeading }\n\n${ fallbackBody }\n${ source.slice(
	...
	const bodyEnd = nextReleaseMatch
		? bodyStart + nextReleaseMatch.index
		: source.length;
...
async function prepareStructuredChanges(
	root,
	currentVersion,
	targetVersion,
	changes
	...
	for ( const filename of [ 'package-lock.json', 'npm-shrinkwrap.json' ] ) {
		...
		if ( updated ) {
			addChange(
				...
				path,
				lock.source,
				stringifyJson( lock.value, lock.source ),
				{
					kind: 'npm-lock-root',
					occurrences: 1,
				}
	...
	if ( await fileExists( composerPath ) ) {
		...
		if (
			composer.value.version !== undefined &&
			composer.value.version !== currentVersion
		) {
			throw new Error(
...
async function prepareTextChanges(
	root,
	currentVersion,
	targetVersion,
	changes
...
async function commitChanges( changes ) {
	...
	try {
		...
		throw new Error(
			`Version update failed and committed files were rolled back: ${ error.message }`
		);
	}
}
...
export async function synchronizeVersion( {
	...
	if ( ! targetVersion ) {
		throw new Error(
			'Target version is required. Provide a version (e.g. "1.3.3"), a bump type ("--bump patch|minor|major"), or set TARGET_VERSION in .env.'
		);
	}
	...
	const changelogMessage =
		changelog ||
		decision ||
		env.TARGET_VERSION_CHANGELOG ||
		env.VERSION_CHANGELOG;
	...
	const phpVersionOrdering = verifyPhpVersionOrdering(
		currentVersion,
		targetVersion
	);

	const changes = new Map();
	await prepareStructuredChanges(
		root,
		currentVersion,
		targetVersion,
		changes
	);
	const { scanned, skipped } = await prepareTextChanges(
		root,
		currentVersion,
		targetVersion,
		changes

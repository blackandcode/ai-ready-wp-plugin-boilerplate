...
function updateDotEnvPassword( password ) {
	...
	try {
		...
		console.log(
			'Synchronized .env with fresh BRUNO_APPLICATION_PASSWORD.'
		);
	} catch ( err ) {
		console.warn(
			'Notice: .env file not found or failed to update:',
			err.message
		);
	}
}
...
try {
	...
	// wp-env maps "." to the directory name
	try {
		...
		const matched = list
			...
			.find(
				( name ) =>
					name.includes( 'boilerplate' ) || name === PLUGIN_SLUG
			);
		if ( matched ) {
			wpInherit( 'plugin', 'activate', matched );
		}
	} catch ( err ) {
		console.warn( 'Plugin activation notice:', err.message );
	}
}

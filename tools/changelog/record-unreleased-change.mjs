...
export function normalizeChangeType( type ) {
	...
	const matched = CANONICAL_TYPES.find(
...
export function recordUnreleasedChange(
	source,
	{ type = 'Changed', message }
) {
	...
	if ( unreleasedIndex === -1 ) {
		const firstLineEnd = working.indexOf( '\n' );
		const insertAt =
			firstLineEnd === -1 ? working.length : firstLineEnd + 1;
	...
	if ( sectionMatch && sectionMatch.index !== undefined ) {
		...
		const sectionContent = unreleasedSection
			.slice( headerStart, sectionContentEnd )
			.trimEnd();
...
Usage:
  ...
  node tools/changelog/record-unreleased-change.mjs [message] [options]
...
if (
	process.argv[ 1 ] &&
	process.argv[ 1 ].endsWith( 'record-unreleased-change.mjs' )
) {
	...
	try {
		...
		console.log(
			`Successfully recorded unreleased [${ result.type }] change in ${ result.path }:`
		);
